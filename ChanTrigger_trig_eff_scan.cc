...
// convert to volts
    if(settings1->WHICH==10){
      Int_t npoints_1 = graph1->GetN();
      Double_t *xpoints_1 = graph1->GetX();
      Double_t *ypoints_1 = graph1->GetY();
      for (int i=0;i<npoints_1;i++){
        ypoints_1[i]*=1.0/1000.0;
      }
      *graph1=TGraph(npoints_1,xpoints_1,ypoints_1);
    }
...
//   if ((iphi ==11 && pol==0) ||(iphi==15 && pol==0)){
//     TCanvas *c = new TCanvas("c");
//     graph1->Draw("Al");
...
// convert to volts
    ...
    if(ant==15){
// debugging, do FFT and check magnitudes for A3 and A4 trigger efficiency scans
    RFSignal *rf = new RFSignal(graph1);
    TCanvas *c2= new TCanvas("c2");
    rf->getFreqMagGraph()->Draw("AL");
    c2->Print("freqmaggraph_graph1.png");
  }
  } else {
    graph1 = new TGraph(nPoints, x, y);
  }
...
//   if ((pol==0 && iphi==15) ||(pol==0 && iphi==11)){
//    TCanvas *c = new TCanvas("c");
//    graph1->Draw("Al");
...
//    if(i==300){
//      cout << "y value of pulser graph at 300 is " << tmp_volts[i] << endl;
//    }
  }
