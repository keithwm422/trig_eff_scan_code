...
void Anita::readTriggerEfficiencyScanPulser(Settings *settings1){
  if(settings1->WHICH==10 || settings1->WHICH==9){
    if(settings1->WHICH==10){
      string fileName = ICEMC_DATA_DIR+"/TriggerEfficiencyScanPulser_anita4.root";
      TFile *f = new TFile(fileName.c_str(), "read");
      gPulseAtAmpa  = (TGraph*)f->Get("Phisector_3_33dBCh1");
...
//        }
        gPulseAtAmpa->GetY()[i]*=TMath::Power(10,-25./20.);
